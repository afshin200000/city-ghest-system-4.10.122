...
if ( ! function_exists( 'cgs_module_applications_enabled' ) ) {
    /**
     * @return bool
     */
    function cgs_module_applications_enabled() {
        $flags = get_option( 'cgs_module_flags', array() );
        if ( ! is_array( $flags ) ) {
            $flags = array();
        }
        // پیش‌فرض: روشن (تا چیزی نشکند)
        if ( ! array_key_exists( 'applications', $flags ) ) {
            return true;
        }
        return ! empty( $flags['applications'] );
    }
}

if ( ! cgs_module_applications_enabled() ) {
    return;
}
...
if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
    // error_log( '[CGS] module applications ready' );
}
