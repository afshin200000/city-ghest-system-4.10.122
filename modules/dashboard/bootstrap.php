...
if ( ! function_exists( 'cgs_module_dashboard_enabled' ) ) {
    /**
     * @return bool
     */
    function cgs_module_dashboard_enabled() {
        $flags = get_option( 'cgs_module_flags', array() );
        if ( ! is_array( $flags ) ) {
            $flags = array();
        }
        // پیش‌فرض: روشن (تا چیزی نشکند)
        if ( ! array_key_exists( 'dashboard', $flags ) ) {
            return true;
        }
        return ! empty( $flags['dashboard'] );
    }
}

if ( ! cgs_module_dashboard_enabled() ) {
    return;
}
...
if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
    // error_log( '[CGS] module dashboard ready' );
}
